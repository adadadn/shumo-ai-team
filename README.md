...
> 一个面向全国大学生数学建模竞赛（CUMCM/高教社杯）的**多 Agent 常备制 AI 团队**完整配置包：7 个角色、25 项已挂载 Skill、三道验收门、证据链门禁与竞赛合规规则，开箱即用、跨赛题复用。
...
| 技能层 | 25 项按角色永久绑定的专业 Skill（文献检索/EDA/建模选型/求解器/图表/写作范式/评审/合规/本地版本快照） |
...
| 文件 | 内容 |
|---|---|
| `TEAM-PACK.md` | 完整可复用配置包：团队章程、架构总则、7 角色身份证、守则与工作链路、25 项 Skill 挂载登记表、外部技能来源清单、需求覆盖跟踪、常驻竞赛规则、运行手册、部署检查清单、附录 A/B（定制技能全文） |
...
*本系统由 AI 多角色协作搭建与验证；配置版本 v1.2（对应团队花名册 v2.8 快照；v1.1 = Skill 来源标注全量核查补全；v1.2 = 新增 local-git-snapshot 本地 Git 版本快照 Skill）。*
