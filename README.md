...
> 一个面向全国大学生数学建模竞赛（CUMCM/高教社杯）的**多 Agent 常备制 AI 团队**完整配置包：7 个角色、26 项已挂载 Skill、三道验收门、证据链门禁与竞赛合规规则，开箱即用、跨赛题复用。
...
*本系统由 AI 多角色协作搭建与验证；配置版本 v1.3（对应团队花名册 v2.9 快照；v1.2 = 新增 local-git-snapshot；v1.3 = 新增 pitfall-log-reuse 踩坑日志复用 Skill）。*
